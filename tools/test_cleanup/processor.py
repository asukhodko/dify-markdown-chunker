...
from .models import (
    TestFile, TestCategorization, DuplicateReport, RemovalReport,
    AdaptationReport, AdaptationPlan, CodeChange, RemovedTest, AdaptedTest
)
...
class TestProcessor(LoggerMixin):
    ...
    def remove_redundant_tests(self, duplicates: DuplicateReport) -> RemovalReport:
        ...
        """
        self.logger.info(f"Removing {len(duplicates.redundant_files)} redundant test files")
        ...
        for file_path in duplicates.redundant_files:
            try:
                ...
                unique_assertions = self.redundancy_detector.extract_unique_assertions(file_path)
                if unique_assertions:
                    preserved_assertions[file_path] = unique_assertions
                    self.logger.info(f"Preserved {len(unique_assertions)} unique assertions from {file_path}")
                ...
                if self.file_manager.remove_file(file_path):
                    ...
                    makefile_changes = self.file_manager.update_makefile_references(file_path)
                    makefile_updates.extend(makefile_changes)
                    
                    self.logger.info(f"Successfully removed redundant test: {file_path}")
                else:
        ...
        self.logger.info(f"Removal complete: {len(removed_files)} files removed, "
                        f"{len(errors)} errors")
    ...
    def adapt_valuable_tests(self, valuable_tests: List[TestFile]) -> AdaptationReport:
        ...
        self.logger.info(f"Adaptation complete: {len(successful_adaptations)} successful, "
...
class RedundancyDetector(LoggerMixin):
    ...
    def _extract_assertion_text(self, assert_node: ast.Assert, content: str) -> Optional[str]:
...
class TestAdapter(LoggerMixin):
    ...
    def create_adaptation_plan(self, test_file: TestFile) -> AdaptationPlan:
        ...
        self.logger.debug(f"Created adaptation plan for {original_path}: "
    ...
    def execute_adaptation(self, plan: AdaptationPlan) -> bool:
        ...
        try:
            ...
            # Apply code changes
            lines = content.splitlines()
            for change in sorted(plan.code_changes, key=lambda x: x.line_number, reverse=True):
                if 0 <= change.line_number - 1 < len(lines):
                    ...
                    if change.old_code in old_line:
                        ...
                        self.logger.debug(f"Applied change at line {change.line_number}: {old_line.strip()} -> {new_line.strip()}")
                    else:
                        self.logger.warning(f"Could not find '{change.old_code}' in line {change.line_number}: {old_line.strip()}")
...
'''
    ...
    def _apply_import_changes_ast(self, content: str, tree: ast.AST, import_changes: Dict[str, str]) -> str:
        """Apply import changes using AST-based replacement."""
        lines = content.splitlines()
        ...
        for start_line, end_line, replacement in sorted(import_replacements, reverse=True):
            # Replace the import lines
            lines[start_line:end_line + 1] = replacement.split('\n')
        
        return '\n'.join(lines)
    
    def _get_import_replacement(self, node: ast.AST, import_changes: Dict[str, str]) -> Optional[str]:
        """Get replacement text for an import node."""
        if isinstance(node, ast.ImportFrom):
            module = node.module or ""
            
            # Check if this is a legacy import that needs replacement
            if 'markdown_chunker_v2' in module or 'markdown_chunker' in module:
                ...
                elif any(name in ['AdaptiveSizeCalculator', 'AdaptiveSizeConfig'] for name in imported_names):
                    # These don't exist in chunkana, skip or comment out
                    return '# Legacy adaptive sizing imports - functionality moved to chunkana core'
                elif any(name in ['Parser', 'StreamingConfig', 'HierarchyBuilder'] for name in imported_names):
                    # These don't exist in chunkana, skip or comment out
    ...
    def _plan_import_changes(self, imports: List[str]) -> Dict[str, str]:
        ...
        for import_stmt in imports:
            ...
            if 'markdown_chunker_v2' in original_stmt or 'markdown_chunker' in original_stmt:
                # Replace with adapter import - keep MarkdownChunker as alias
                if 'MarkdownChunker' in original_stmt and 'ChunkConfig' in original_stmt:
                    # Multiple imports: from module import MarkdownChunker, ChunkConfig
                    import_changes[original_stmt] = 'from adapter import MarkdownChunker\nfrom chunkana import ChunkerConfig as ChunkConfig'
                elif 'MarkdownChunker' in original_stmt:
                    # Single MarkdownChunker import
                    import_changes[original_stmt] = 'from adapter import MarkdownChunker'
                elif 'ChunkConfig' in original_stmt:
                    # ChunkConfig import
                    import_changes[original_stmt] = 'from chunkana import ChunkerConfig as ChunkConfig'
