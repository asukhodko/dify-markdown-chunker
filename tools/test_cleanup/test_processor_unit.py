...
"""
            ...
            complex_assertions = [a for a in assertions if 'isinstance' in a or 'len(' in a or '!=' in a]
...
"""
            ...
            analysis = TestAnalysis(
                ...
                imports=["from markdown_chunker_v2 import MarkdownChunker", "import pytest"],
...
"""
            ...
            plan = AdaptationPlan(
                ...
                import_changes={
                    "from markdown_chunker_v2 import MarkdownChunker": "from adapter import MigrationAdapter"
                },
                code_changes=[
                    CodeChange(
            ...
            # Check that legacy imports are replaced in the actual code (not in comments)
...
"""
            ...
            assert "test_remove_me.py" not in updated_content, "References should be removed"
...
""")
            ...
            assert len(removal_report.preserved_assertions) > 0, "Should preserve assertions"
            
            # Verify files are actually removed
            for test_file in test_files:
                full_path = os.path.join(temp_dir, test_file)
                assert not os.path.exists(full_path), f"File should be removed: {test_file}"
