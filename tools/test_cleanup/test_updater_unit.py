...
"""
        ...
        updated_content, changes = self.updater._remove_file_references(
            content, "test_remove_me.py"
        )
...
"""
            ...
            changes = ChangeReport(
                removed_tests=["test1.py", "test2.py"], adapted_tests=["test3.py"]
            )
...
"""
    ...
    def test_update_docs_integration(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            changes = ChangeReport(
                removed_tests=["test1.py"], adapted_tests=["test2.py"]
            )
...
class TestInfrastructureUpdater:
    ...
    def test_update_makefile_integration(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            self.config.project_root = temp_dir
            self.updater.makefile_updater.makefile_path = os.path.join(
                temp_dir, "Makefile"
            )
    ...
    def test_update_pytest_config_integration(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            self.config.project_root = temp_dir
            self.updater.pytest_updater.pytest_ini_path = os.path.join(
                temp_dir, "pytest.ini"
            )
            self.updater.pytest_updater.pyproject_path = os.path.join(
    ...
    def test_update_documentation_integration(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            self.updater.doc_updater.contributing_path = os.path.join(
                temp_dir, "CONTRIBUTING.md"
            )
