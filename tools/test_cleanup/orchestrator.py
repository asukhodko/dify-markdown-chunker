...
from .logging_setup import LoggerMixin, setup_logging
from .models import AdaptedTest, CleanupReport, RemovedTest
...
class CleanupOrchestrator(LoggerMixin):
    ...
    def run_cleanup(self) -> CleanupReport:
        ...
        try:
            ...
            self.logger.info(
                f"Analysis complete: {len(analyses)} files analyzed, "
            ...
            adaptation_report = self.processor.adapt_valuable_tests(
                categorization.tests_to_adapt
            )

            self.logger.info(
                f"Processing complete: {len(removal_report.removed_files)} removed, "
                f"{len(adaptation_report.successful_adaptations)} adapted"
            )

            # Phase 3: Infrastructure Updates
            self.logger.info("Phase 3: Updating infrastructure")
            self.updater.update_makefile(
                removal_report.removed_files, adaptation_report.adapted_files
            )
            ...
            change_report = ChangeReport(
                removed_tests=[
                    RemovedTest(
                        file_path=f, test_functions=[], removal_reason="redundant"
                    )
                ...
                adapted_tests=[
                    ...
                    for f in adaptation_report.successful_adaptations
                ],
            )
            ...
            self.logger.info(
                f"Cleanup completed successfully in {execution_time:.2f} seconds"
            )
        ...
        try:
            ...
            if not self.config.dry_run:
                ...
                if test_dir.exists():
                    ...
                    if migration_tests:
                        self.logger.info(
                            f"Found {len(migration_tests)} migration tests"
                        )
                        return True
    ...
    def _print_summary(self, report: CleanupReport) -> None:
        ...
        print(
            ...
            f"{report.summary.coverage_maintained_percent:.1f}%"
        )
        print(
            f"Execution time:              {report.summary.execution_time_seconds:.2f}s"
        )
...
Examples:
  ...
  python -m tools.test_cleanup.orchestrator --verbose --no-backups
    ...
    parser.add_argument(
        "--output-dir", help="Directory for cleanup reports (default: cleanup_reports)"
    )
