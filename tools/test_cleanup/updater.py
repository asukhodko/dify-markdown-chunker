...
class InfrastructureUpdater(LoggerMixin):
    ...
    def update_makefile(
        self, removed_files: List[str], adapted_files: List[str]
    ) -> None:
        """
        ...
        """
        self.logger.info(
            f"Updating Makefile for {len(removed_files)} removed and "
            f"{len(adapted_files)} adapted files"
        )
    ...
    def update_pytest_config(self, excluded_dirs: List[str]) -> None:
        ...
        """
        self.logger.info(
            f"Updating pytest config to exclude {len(excluded_dirs)} directories"
        )
...
class MakefileUpdater(LoggerMixin):
    ...
    def update_targets(
        ...
        try:
            ...
            for removed_file in removed_files:
                content, file_changes = self._remove_file_references(
                    content, removed_file
                )
                changes.extend(file_changes)

            # Add references to adapted files
            for adapted_file in adapted_files:
                content, file_changes = self._add_adapted_file_references(
                    content, adapted_file
                )
                changes.extend(file_changes)
    ...
    def _remove_file_references(
        self, content: str, removed_file: str
    ) -> tuple[str, List[str]]:
        ...
        for line in lines:
            ...
            if file_name in line or file_stem in line:
                ...
                if line != original_line:
                    changes.append(
                        f"Removed {file_name} reference: "
                        f"{original_line.strip()} -> {line}"
                    )
    ...
    def _add_adapted_file_references(
        self, content: str, adapted_file: str
    ) -> tuple[str, List[str]]:
    ...
    def _optimize_test_targets(self, content: str) -> tuple[str, List[str]]:
        ...
        for line in lines:
            ...
            if (
                line.strip().startswith("python -m pytest tests/")
                and "test-all:" in content
            ):
                # Ensure test-all runs all tests but excludes problematic ones
                if "tests/" in line and not any(
                    exclude in line for exclude in ["--ignore", "-k"]
                ):
                    line = line.replace(
                        "tests/", "tests/ --ignore=tests/legacy_removed/"
                    )
                    if line != original_line:
                        changes.append(
                            f"Optimized test-all target: "
...
class PytestConfigUpdater(LoggerMixin):
    ...
    def _update_pytest_ini(self, excluded_dirs: List[str]) -> List[str]:
        ...
        try:
            ...
            # Process the content
            lines = content.splitlines()
            modified_lines, changes = self._process_pytest_lines(lines, excluded_dirs)
            updated_content = "\n".join(modified_lines)

            # Write changes if needed
            if updated_content != original_content:
                self._write_pytest_changes(updated_content, changes)
    ...
    def _process_pytest_lines(
        self, lines: List[str], excluded_dirs: List[str]
    ) -> tuple[List[str], List[str]]:
        """Process pytest.ini lines to add ignore patterns."""
        modified_lines = []
        changes: List[str] = []
        ...
        for line in lines:
            if self._is_pytest_section_start(line):
                ...
                if not added_ignores and excluded_dirs:
                    ignore_line = self._create_ignore_line(excluded_dirs)
                ...
                updated_line = self._update_addopts_line(line, excluded_dirs)
                modified_lines.append(updated_line)
                changes.append(f"Updated addopts: {updated_line}")
                added_ignores = True
            else:
                modified_lines.append(line)

        # Add pytest section if it doesn't exist
        if not self._has_pytest_section(lines):
            section_lines = self._create_pytest_section(excluded_dirs)
            modified_lines.extend(section_lines)
            changes.append("Added [tool:pytest] section with ignore patterns")

        return modified_lines, changes

    def _is_pytest_section_start(self, line: str) -> bool:
        """Check if line starts a pytest section."""
        return line.strip() in ["[tool:pytest]", "[pytest]"]

    def _create_ignore_line(self, excluded_dirs: List[str]) -> str:
        """Create ignore line for excluded directories."""
        return f"addopts = --ignore={' --ignore='.join(excluded_dirs)}"

    def _update_addopts_line(self, line: str, excluded_dirs: List[str]) -> str:
        """Update existing addopts line with new ignore patterns."""
        for excluded_dir in excluded_dirs:
            if f"--ignore={excluded_dir}" not in line:
                line += f" --ignore={excluded_dir}"
        return line

    def _has_pytest_section(self, lines: List[str]) -> bool:
        """Check if pytest section exists in lines."""
        return any("[pytest]" in line or "[tool:pytest]" in line for line in lines)

    def _create_pytest_section(self, excluded_dirs: List[str]) -> List[str]:
        """Create new pytest section with ignore patterns."""
        ignore_opts = (
            f"addopts = --ignore={' --ignore='.join(excluded_dirs)}"
            if excluded_dirs
            else "addopts = -v"
        )
        return ["", "[tool:pytest]", ignore_opts]

    def _write_pytest_changes(self, content: str, changes: List[str]) -> None:
        """Write pytest.ini changes to file."""
        if not self.config.dry_run:
            ...
            self.logger.info(
                f"DRY RUN: Would update pytest.ini with {len(changes)} changes"
            )
...
testpaths = tests
addopts = -v --tb=short{ignore_opts}
...
class DocumentationUpdater(LoggerMixin):
    ...
    def _update_readme(self, changes: ChangeReport) -> List[str]:
        ...
        try:
            ...
            removed_count = len(getattr(changes, "removed_tests", []))
...
- **Migration-compatible tests**: Tests that work with the current
  adapter-based architecture
- **Adapted tests**: Legacy tests that have been adapted to use the migration adapter
- **Removed tests**: {removed_count} redundant tests were removed during cleanup
...
"""
            ...
            if "## Testing" in content:
                ...
                pattern = r"## Testing.*?(?=\n## |\n# |\Z)"
                content = re.sub(
                    pattern, testing_section.strip(), content, flags=re.DOTALL
                )
            ...
            if content != original_content and not self.config.dry_run:
                ...
                self.logger.info(
                    "DRY RUN: Would update README.md with test structure information"
                )
...
1. **Use the migration adapter**: New tests should use `MigrationAdapter`
   instead of legacy modules
...
"""
            ...
            if "## Test Guidelines" in content:
                ...
                content = re.sub(
                    pattern, guidelines_section.strip(), content, flags=re.DOTALL
                )
            ...
            if content != original_content and not self.config.dry_run:
                ...
                self.logger.info(
                    "DRY RUN: Would update CONTRIBUTING.md with test guidelines"
                )
    ...
    def _update_testing_docs(self, changes: ChangeReport) -> List[str]:
        ...
        try:
            removed_count = len(getattr(changes, "removed_tests", []))
            adapted_count = len(getattr(changes, "adapted_tests", []))
...
"""
            ...
            if not self.config.dry_run:
                ...
                self.logger.info(
                    "DRY RUN: Would create/update TESTING.md documentation"
                )
