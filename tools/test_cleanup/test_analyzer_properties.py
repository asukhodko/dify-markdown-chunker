...
from .analyzer import CoverageAnalyzer, ImportAnalyzer, TestAnalyzer
from .config import CleanupConfig
from .models import TestAnalysis, TestCategory, TestType
        ...
        for file_path, analysis in self.analyzed_files.items():
            ...
            for func_name in analysis.test_functions:
                assert func_name.startswith(
                    "test_"
                ), f"Invalid test function name: {func_name}"
...
def test_property_1_categorization_accuracy(test_contents: List[str]):
    ...
    (those with unique coverage), and test types (unit, integration,
    performance, property-based).
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        all_tests = (
            categorization.redundant_tests
        ...
        assert len(all_paths) == len(
            set(all_paths)
        ), "Tests appear in multiple categories"
        ...
        for test in all_tests:
            if test.category == TestCategory.MIGRATION_COMPATIBLE:
                assert (
                    not test.analysis.has_legacy_imports
                ), "Migration-compatible test has legacy imports"
...
def test_property_duplicate_detection_consistency(test_contents: List[str]):
    ...
    For any set of test files, duplicate detection should be consistent and
    symmetric. If test A is a duplicate of test B, then they should have
    significant coverage overlap.
    """
    with tempfile.TemporaryDirectory() as temp_dir:
        config = CleanupConfig.from_project_root(temp_dir)
        analyzer = TestAnalyzer(config)
        ...
        for legacy_path, migration_path in duplicate_report.duplicate_pairs:
            ...
            migration_analysis = next(
                a for a in analyses if a.file_path == migration_path
            )
            ...
            if legacy_coverage and migration_coverage:
                ...
                assert (
                    overlap_ratio >= config.duplicate_similarity_threshold
...
@settings(
    max_examples=50, deadline=3000, suppress_health_check=[HealthCheck.filter_too_much]
)
def test_property_import_analysis_correctness(content: str):
    """
    Property: Import Analysis Correctness

    For any Python content, import analysis should correctly identify all
    import statements and detect legacy imports based on configured patterns.
    """
...
"""
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        for import_stmt in imports:
            assert import_stmt.startswith(
                ("import ", "from ")
            ), f"Invalid import: {import_stmt}"

        # Property: Legacy detection should be consistent with patterns
        import_text = " ".join(imports)
        expected_legacy = any(
            pattern in import_text for pattern in config.legacy_import_patterns
        )
...
@settings(
    max_examples=20, deadline=3000, suppress_health_check=[HealthCheck.filter_too_much]
)
def test_property_coverage_analysis_completeness(content: str):
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        assert len(coverage_areas) == len(
            set(coverage_areas)
        ), "Duplicate coverage areas found"
...
def test_property_end_to_end_analysis_workflow(num_files: int):
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        total_categorized = (
            len(categorization.redundant_tests)
        ...
        assert isinstance(
            duplicate_report.duplicate_pairs, list
        ), "Invalid duplicate pairs"
        assert isinstance(
            duplicate_report.redundant_files, list
        ), "Invalid redundant files"
        assert isinstance(
            duplicate_report.unique_coverage_areas, list
        ), "Invalid unique coverage areas"
