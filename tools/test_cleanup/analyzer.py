...
class TestAnalyzer(LoggerMixin):
    ...
    def analyze_test_file(self, file_path: str) -> TestAnalysis:
        ...
        self.logger.debug(
            f"Analysis complete for {file_path}: {len(test_functions)} tests, "
    ...
    def _determine_test_type(
        ...
        if "hypothesis" in content.lower() or "@given" in content:
            ...
            for keyword in ["integration", "end-to-end", "e2e"]
        ):
            return TestType.INTEGRATION
    ...
    def _calculate_complexity(self, tree: ast.AST) -> float:
        ...
        return int(complexity) if complexity > 0 else 0
...
class TestCategorizer(LoggerMixin):
    ...
    def categorize(self, analyses: List[TestAnalysis]) -> TestCategorization:
        ...
        self.logger.info(
            f"Categorization complete: {len(redundant_tests)} redundant, "
    ...
    def identify_duplicates(
        ...
        for legacy_test in legacy_tests:
            ...
            overlap_ratio = (
                len(overlap) / len(legacy_coverage) if legacy_coverage else 0
            )

            if overlap_ratio >= self.config.duplicate_similarity_threshold:
                # Find the most similar migration test
                best_match = self._find_best_match(
                    legacy_test, categorization.migration_compatible_tests
                )
                if best_match:
    ...
    def _find_best_match(
        self, legacy_test: TestFile, migration_tests: List[TestFile]
    ) -> Optional[TestFile]:
