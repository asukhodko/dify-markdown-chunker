...
"""
    ...
    def test_extract_coverage_from_function_name(self):
        ...
        test_cases = [
            (
                "test_chunk_markdown_text",
                ["chunking", "markdown_processing", "text_processing"],
            ),
            ("test_parse_config_file", ["parsing", "configuration", "file"]),
            (
                "test_error_handling_invalid_input",
                ["error_handling", "invalid", "input_processing"],
            ),
        ]
...
"""
            tree = ast.parse(content)
            test_type = analyzer._determine_test_type(
                "test_performance.py", tree, content
            )
...
"""
            tree = ast.parse(content)
            test_type = analyzer._determine_test_type(
                "test_integration_basic.py", tree, content
            )
...
"""
    ...
    def test_should_include_file(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            assert (
                analyzer._should_include_file("tests/test_migration_adapter.py")
                is False
            )
