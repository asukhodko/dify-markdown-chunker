...
These tests validate universal correctness properties of the infrastructure update system.
"""
...
"""
        ...
        if os.path.exists(makefile_path):
            ...
            for removed_file in removed_files:
                ...
                assert file_name not in updated_makefile, f"Removed file {file_name} still referenced in Makefile"
        ...
        if os.path.exists(pytest_ini_path):
            ...
            for excluded_dir in excluded_dirs:
                if excluded_dir:  # Only check non-empty directories
                    assert "--ignore=" in updated_pytest_ini or excluded_dir in updated_pytest_ini, \
        ...
        original_makefile = open(makefile_path, 'r').read() if os.path.exists(makefile_path) else ""
        original_pytest = open(pytest_ini_path, 'r').read() if os.path.exists(pytest_ini_path) else ""
        
        # Run updates again
        updater.update_makefile(removed_files, adapted_files)
        updater.update_pytest_config(excluded_dirs)
        
        final_makefile = open(makefile_path, 'r').read() if os.path.exists(makefile_path) else ""
        final_pytest = open(pytest_ini_path, 'r').read() if os.path.exists(pytest_ini_path) else ""
        
        assert original_makefile == final_makefile, "Makefile updates should be idempotent"
        assert original_pytest == final_pytest, "pytest.ini updates should be idempotent"
...
"""
        ...
        changes = makefile_updater.update_targets(files_to_remove, [])
        ...
        for removed_file in files_to_remove:
            assert removed_file not in updated_content, f"Removed file {removed_file} still in Makefile"
        
        # Property: Kept files should still be referenced
        for kept_file in files_to_keep:
            if kept_file:  # Only check non-empty file names
                assert kept_file in updated_content, f"Kept file {kept_file} was incorrectly removed"
        
        # Property: Other content should be preserved
        assert "test_keep.py" in updated_content, "Unrelated files should be preserved"
        assert "other-target:" in updated_content, "Unrelated targets should be preserved"
        assert "This should be preserved" in updated_content, "Unrelated content should be preserved"


@given(st.lists(st.text(min_size=1, max_size=15, alphabet=st.characters(whitelist_categories=('Lu', 'Ll'))), min_size=0, max_size=4))
@settings(max_examples=15, deadline=5000)
def test_property_pytest_config_exclusion(excluded_dirs: List[str]):
    ...
    excluded_dirs = list(set(dir_name for dir_name in excluded_dirs if dir_name.strip()))
...
"""
        ...
        changes = pytest_updater.update_config(excluded_dirs)
        ...
        for excluded_dir in excluded_dirs:
            if excluded_dir:  # Only check non-empty directories
                assert f"--ignore={excluded_dir}" in updated_content or excluded_dir in updated_content, \
                    f"Excluded directory {excluded_dir} should be in ignore patterns"
        
        # Property: Original settings should be preserved
        assert "testpaths = tests" in updated_content, "Original testpaths should be preserved"
        assert "python_files = test_*.py" in updated_content, "Original python_files should be preserved"
        
        # Property: Configuration should remain valid
        assert "[tool:pytest]" in updated_content or "[pytest]" in updated_content, \
...
"""
        ...
        changes = ChangeReport(
            removed_tests=[f"test_removed_{i}.py" for i in range(num_removed)],
            adapted_tests=[f"test_adapted_{i}.py" for i in range(num_adapted)]
        )
        ...
        if num_removed > 0:
            assert str(num_removed) in updated_readme, f"Should mention {num_removed} removed tests"
        ...
        assert "# Testing Documentation" in testing_content, "TESTING.md should have proper header"
        assert "Test Suite Overview" in testing_content, "Should have overview section"
        assert str(num_removed) in testing_content, f"Should mention {num_removed} removed tests"
        assert str(num_adapted) in testing_content, f"Should mention {num_adapted} adapted tests"
