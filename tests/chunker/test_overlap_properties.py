    ...
    **Validates: Requirements 3.4**
    ...
    for chunk in overlapped:
        if chunk.get_metadata("has_overlap", False):
            ...
            assert actual_size <= overlap_size * 1.5, \
    ...
    **Validates: Requirements 3.4, 3.5**
    ...
    for i, chunk in enumerate(overlapped):
        if i == 0:
            ...
            assert not chunk.get_metadata("has_overlap", False), \
                "First chunk should not have overlap"
        else:
            ...
            if chunk.get_metadata("has_overlap", False):
                ...
                assert chunk.metadata["overlap_size"] > 0, \
                    "Overlap size should be positive"
                assert chunk.metadata["overlap_type"] == "prefix", \
        ...
        if chunk.get_metadata("has_overlap", False):
            ...
            if len(parts) > 0:
                ...
                if overlap_text.strip():
                    ...
                    chunk_content_size = len(chunk.content) - overlap_size_actual - 2  # subtract overlap and \n\n
...
def test_property_overlap_statistics_accurate(chunks, overlap_size):
    ...
    **Validates: Requirements 3.4**
    ...
    # Verify statistics
    assert stats["total_chunks"] == len(overlapped), \
    ...
    assert stats["chunks_with_overlap"] == chunks_with_overlap, \
    ...
    if chunks_with_overlap > 0:
        assert stats["avg_overlap_size"] > 0, \
            "Average overlap size should be positive"
        assert stats["total_overlap_size"] > 0, \
            "Total overlap size should be positive"
