...
from markdown_chunker.parser.types import Stage1Results, ContentAnalysis
...
def mock_stage1_results(draw):
    ...
    mock.analysis.content_type = draw(st.sampled_from(['text_heavy', 'code_heavy', 'mixed']))
...
def test_property_fallback_produces_valid_chunks_on_failure(content, stage1):
    ...
    **Validates: Requirements 1.3, 2.2**
    ...
    assert result.fallback_level in [1, 2], "Should use fallback level 1 (structural) or 2 (sentences)"
    assert result.strategy_used in ["structural", "sentences"], "Should use structural or sentences strategy"
...
def test_property_fallback_metadata_correct(content, stage1):
    ...
    **Validates: Requirements 1.3, 2.2**
    ...
    for chunk in result.chunks:
        ...
        assert chunk.metadata["fallback_level"] in [1, 2], "Fallback level should be 1 or 2"
        ...
        assert any(s in chunk.metadata["fallback_reason"] for s in ["structural", "sentences"]), \
...
def test_property_empty_chunks_trigger_fallback(content, stage1):
    ...
    **Validates: Requirements 1.3, 6.1**
    ...
    assert result.fallback_used is True, "Fallback should be used when primary returns empty"
    assert len(result.chunks) > 0, "Fallback should produce chunks"
...
def test_property_fallback_preserves_content(content, stage1):
    ...
    **Validates: Requirements 1.5, 10.2**
    ...
    if content_words:
        ...
        assert preserved_ratio >= 0.5, f"Should preserve at least 50% of content, got {preserved_ratio:.2%}"
...
def test_property_fallback_is_deterministic(content, stage1):
    ...
    **Validates: Requirements 10.2, 10.5**
    ...
    # Verify determinism
    assert len(result1.chunks) == len(result2.chunks), "Should produce same number of chunks"
    assert result1.strategy_used == result2.strategy_used, "Should use same strategy"
    assert result1.fallback_level == result2.fallback_level, "Should use same fallback level"
...
def test_fallback_skipped_when_primary_is_sentences():
    ...
    assert result.fallback_used is False, "Should not use fallback when primary is sentences"
    assert result.strategy_used == "sentences", "Should use sentences strategy"
