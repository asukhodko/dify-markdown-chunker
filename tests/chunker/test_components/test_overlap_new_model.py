...
class TestOverlapNewModel:
    ...
    def test_legacy_mode_content_merge(self):
        ...
        # First chunk stays same (no previous)
        assert result[0].content == "First chunk content here."
        
        # Second chunk should have merged content (longer than original)
        # It should include context from first chunk
        assert len(result[1].content) > len("Second chunk content here.")
        
        # Context fields should NOT be in metadata
    ...
    def test_mode_equivalence(self):
        ...
        for i in range(len(original_chunks)):
            ...
            assert composed == legacy_chunk.content, f"Chunk {i}: Mode equivalence failed"
