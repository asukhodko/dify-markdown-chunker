...
This is the conclusion."""
        ...
        meta_chunks = meta_result.chunks if hasattr(meta_result, 'chunks') else meta_result
        legacy_chunks = legacy_result.chunks if hasattr(legacy_result, 'chunks') else legacy_result
...
Another short section."""
        ...
        for chunk in chunks:
            ...
            if chunk.content.startswith("# Section"):
                assert "previous_content" not in chunk.metadata or \
                       chunk.metadata.get("previous_content") == ""
...
The chunker supports multiple strategies."""
        ...
        # Validate context traceability
        for i in range(1, len(chunks)):
            chunk = chunks[i]
            
            # If has previous_content, it should be from previous chunk
            if "previous_content" in chunk.metadata:
                prev_context = chunk.metadata["previous_content"]
                prev_chunk = chunks[i - 1]
                
                # Context should be substring of previous chunk
                assert prev_context in prev_chunk.content, \
                    f"Chunk {i}: previous_content not found in previous chunk"
                
                # Should be from the end of previous chunk
                assert prev_chunk.content.endswith(prev_context) or \
                       prev_context in prev_chunk.content[-len(prev_context)-20:], \
                    f"Chunk {i}: previous_content not from end of previous chunk"
...
```"""
        ...
        for chunk in chunks:
            ...
            assert fence_count % 2 == 0, f"Unbalanced code fences in chunk"
