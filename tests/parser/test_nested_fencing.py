...
Tests comprehensive fence detection including:
- Quadruple and quintuple backticks
- Tilde fencing (~~~, ~~~~, ~~~~~)
- Mixed fence types
- Edge cases (unclosed, indented, trailing spaces)
...
````"""
    ...
    def test_quadruple_closes_with_quadruple(self):
        """Verify quadruple backticks close correctly with >= 4 backticks."""
        text = """````markdown
content
````"""
        parser = Parser()
    ...
    def test_quadruple_closes_with_quintuple(self):
        """Verify quadruple can close with quintuple (5 >= 4)."""
        text = """````markdown
content
`````"""
        parser = Parser()
...
class TestQuintupleBackticks:
    """Tests for quintuple backtick deep nesting."""

    def test_quintuple_backticks_detection(self):
        """Verify quintuple-backtick opening fence is detected."""
        text = """`````markdown
````markdown
```python
code
...
`````"""
        ...
        block = result.code_blocks[0]
        assert block.fence_char == "`"
        assert block.fence_length == 5
        assert block.is_closed is True

    def test_triple_nesting_preserved(self):
        """Verify triple nesting (5-level, 4-level, 3-level) preserved."""
        text = """`````markdown
````markdown
```python
def nested():
...
`````"""
        ...
        assert len(result.code_blocks) == 1
        block = result.code_blocks[0]
        # Verify all nested levels are in content
        assert "````markdown" in block.content
        assert "```python" in block.content
        assert "def nested():" in block.content


class TestFenceLengthVariations:
    """Tests for various fence lengths."""

    def test_fence_length_3_to_7(self):
        """Test fence lengths from 3 to 7 backticks."""
...
{fence}"""
            ...
            assert len(result.code_blocks) == 1
            block = result.code_blocks[0]
            assert block.fence_length == length
            assert block.is_closed is True

    def test_closing_fence_must_be_equal_or_longer(self):
        """Verify closing fence must have >= opening length."""
        # 4-backtick opening, 3-backtick line inside (doesn't close)
        text = """````python
```
code
```
````"""
        ...
        block = result.code_blocks[0]
        assert block.fence_length == 4
        # Verify the triple-backtick lines are in content (didn't close fence)
        assert "```" in block.content

    def test_longer_closing_fence_works(self):
        """Verify closing fence can be longer than opening."""
        text = """````python
code
``````"""  # 6 backticks close 4-backtick opening
        parser = Parser()
        result = parser.analyze(text)
...
~~~~"""
    ...
    def test_tilde_quintuple(self):
        """Verify quintuple-tilde fences work."""
        text = """~~~~~markdown
~~~~python
~~~code
content
...
~~~~"""
    ...
    def test_different_fence_types_dont_cross_close(self):
        """Verify backtick doesn't close tilde and vice versa."""
        text = """~~~~markdown
...
~~~~"""
        ...
        block = result.code_blocks[0]
        assert block.fence_char == "~"
        assert "```backticks```" in block.content
        assert block.is_closed is True
...
```   """  # Trailing spaces after closing fence
    ...
    def test_indented_fences(self):
        """Verify fences with leading whitespace are recognized."""
        text = """  ```python
  code
  ```"""
        parser = Parser()
...
```"""
    ...
    def test_language_specifier_variations(self):
        """Test various language specifier formats."""
        test_cases = [
            ("```python", "python"),
            ("```py", "py"),
            ("```123", "123"),
        ...
        for fence_line, expected_lang in test_cases:
            text = f"""{fence_line}
code
```"""
            parser = Parser()
            ...
            assert len(result.code_blocks) == 1
            assert result.code_blocks[0].language == expected_lang
...
```"""
        ...
        assert len(result.code_blocks) == 1
        content = result.code_blocks[0].content
        assert content.count("\n") == 2  # Two newlines = 3 lines
        assert "\n\n" in content  # Empty line preserved

    def test_nested_content_intact(self):
        """Verify inner fence syntax preserved exactly."""
        text = """````markdown
```python
def test():
...
````"""
        ...
        assert len(result.code_blocks) == 1
        content = result.code_blocks[0].content
        # Verify no normalization of nested fences
        assert content.count("```") == 2
        assert "```python" in content
        assert "def test():" in content

    def test_line_breaks_preserved(self):
        """Verify line breaks within nested fences preserved."""
        text = """````markdown
First line
```
middle
...
{fence}"""
    ...
    def test_is_closed_true_for_closed_fence(self):
        """Verify is_closed=True for properly closed fences."""
        text = """```python
code
```"""
        parser = Parser()
    ...
    def test_is_closed_false_for_unclosed_fence(self):
        """Verify is_closed=False for unclosed fences."""
        text = """```python
unclosed"""
        parser = Parser()
        result = parser.analyze(text)
...
````"""
    ...
    def test_nested_with_headers_and_text(self):
        """Test nested fences mixed with headers and text."""
        text = """# Documentation
...
That's how it works."""
        ...
        # Should extract 1 code block (the quadruple-backtick one)
        assert len(result.code_blocks) == 1
        block = result.code_blocks[0]
        assert block.fence_length == 4
