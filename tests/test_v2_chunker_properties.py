...
Tests MarkdownChunker functionality including:
- Basic chunking
...
"""
    ...
    @settings(max_examples=50, deadline=5000, suppress_health_check=[HealthCheck.too_slow, HealthCheck.filter_too_much])
    ...
    **Reference**: docs/v2-test-specification/v2-test-specification.md#SPEC-011
    ...
    def test_no_overlap_when_disabled(self):
        ...
        for chunk in chunks:
            ...
            assert 'previous_content' not in chunk.metadata or chunk.metadata.get('previous_content') is None
    
    @given(st.integers(min_value=10, max_value=100))
    @settings(max_examples=20, deadline=5000)
    def test_overlap_size_respected(self, overlap_size: int):
        """Property: Overlap size is approximately respected."""
        config = ChunkConfig(max_chunk_size=500, min_chunk_size=50, overlap_size=overlap_size)
        chunker = MarkdownChunker(config)
