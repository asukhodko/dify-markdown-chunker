    ...
    **Validates: Requirements 3.1**
    ...
    def test_chunking_consistent_regardless_of_line_endings(self, md_text: str):
        ...
        **Validates: Requirements 3.1**
        ...
        assert len(chunks_original) == len(
            chunks_unix
...
"""
        ...
        for chunk in chunks:
            if chunk.size > config.max_chunk_size:
                ...
                ), "Oversize chunk missing allow_oversize flag"
                ...
                assert reason in valid_reasons, f"Invalid oversize_reason: {reason}"
...
class TestProp2OverlapCompleteness:
    ...
    **Validates: Requirements 2.1, 2.2, 2.7, 2.8**
    ...
    def test_overlap_completeness(self, md_text: str):
        ...
        **Validates: Requirements 2.1, 2.2, 2.7, 2.8**
        """
        config = ChunkConfig(max_chunk_size=200, min_chunk_size=50, overlap_size=40)
        chunker = MarkdownChunker(config)
        ...
        assert (
            "next_content" not in chunks[-1].metadata
        ...
        for i in range(1, len(chunks)):
            ...
            assert (
                prev_content in chunks[i - 1].content
        ...
        for i in range(len(chunks) - 1):
            ...
            assert (
                next_content in chunks[i + 1].content
    ...
    **Validates: Requirements 2.3, 2.6**
    ...
    def test_overlap_size_constraint(self, md_text: str):
        ...
        **Validates: Requirements 2.3, 2.6**
        ...
        overlap_size = 50
        config = ChunkConfig(
            max_chunk_size=300, min_chunk_size=50, overlap_size=overlap_size
        )
        chunker = MarkdownChunker(config)
        ...
        for i, chunk in enumerate(chunks):
            ...
            if "next_content" in chunk.metadata:
                ...
                assert (
                    next_len <= max_size_with_tolerance
    ...
    **Validates: Requirements 1.1, 1.5**
    ...
    def test_min_chunk_size_constraint(self, md_text: str):
        ...
        **Validates: Requirements 1.1, 1.5**
        ...
        min_size = 100
        config = ChunkConfig(
            max_chunk_size=500, min_chunk_size=min_size, overlap_size=0
        )
        chunker = MarkdownChunker(config)
    ...
    def test_metadata_completeness(self, md_text: str):
        ...
        **Validates: Requirements 3.1, 3.2, 3.3, 3.4**
        ...
        for i, chunk in enumerate(chunks):
            ...
            # content_type
            assert "content_type" in chunk.metadata, f"Chunk {i} missing content_type"
            assert (
                chunk.metadata["content_type"] in valid_content_types
            ), f"Chunk {i} has invalid content_type: {chunk.metadata['content_type']}"

            # has_code
            assert "has_code" in chunk.metadata, f"Chunk {i} missing has_code"
            assert isinstance(
                chunk.metadata["has_code"], bool
            ), f"Chunk {i} has_code is not boolean"

            # header_path
            assert "header_path" in chunk.metadata, f"Chunk {i} missing header_path"
            assert isinstance(
                chunk.metadata["header_path"], list
    ...
    def test_serialization_roundtrip(self, md_text: str):
        ...
        **Validates: Requirements 6.1, 6.2, 6.3**
        ...
        for i, chunk in enumerate(chunks):
            ...
            assert (
                restored.start_line == chunk.start_line
            ), f"Chunk {i}: start_line differs after round-trip"
            assert (
                restored.end_line == chunk.end_line
            ), f"Chunk {i}: end_line differs after round-trip"
            assert (
                restored.metadata == chunk.metadata
...
class TestProp6MetricsConsistency:
    ...
    **Validates: Requirements 5.1, 5.2, 5.3, 5.4**
    ...
    def test_metrics_consistency(self, md_text: str):
        ...
        **Validates: Requirements 5.1, 5.2, 5.3, 5.4**
        """
        config = ChunkConfig(max_chunk_size=500, min_chunk_size=100, overlap_size=0)
        chunker = MarkdownChunker(config)
        ...
        # total_chunks
        assert metrics.total_chunks == len(
            chunks
        ...
        assert (
            metrics.undersize_count == actual_undersize
        ...
        assert (
            metrics.oversize_count == actual_oversize
