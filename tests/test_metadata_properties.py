    ...
    **Validates: Requirements 1.1, 1.3, 1.5**
    ...
    def test_header_path_ends_with_first_header(self, md_text):
        ...
        **Validates: Requirements 1.1, 1.3, 1.5**
        ...
        config = ChunkConfig(
            max_chunk_size=500, min_chunk_size=50, structure_threshold=2
        )
    ...
    **Validates: Requirements 2.1, 2.2, 2.3, 2.4**
    ...
    def test_preamble_creates_separate_chunk(self, md_text):
        ...
        **Validates: Requirements 2.1, 2.2, 2.3, 2.4**
        ...
        assert (
            len(preamble_chunks) >= 1
        ), "Document with preamble should have preamble chunk"
...
class TestProperty4NonEmptyHeaderPath:
    ...
    **Validates: Requirements 3.1, 3.3**
    ...
    def _has_header_in_content(self, content):
        """Check if content has headers outside code blocks."""
        in_code_block = False
        for line in content.split("\n"):
            if line.strip().startswith("```"):
                in_code_block = not in_code_block
    ...
    def _has_structural_header(self, content):
        """Check if content has H1 or H2 headers outside code blocks."""
        in_code = False
        for line in content.split("\n"):
            if line.strip().startswith("```"):
                in_code = not in_code
    ...
    def test_chunks_with_headers_have_nonempty_path(self, md_text):
        ...
        **Validates: Requirements 3.1, 3.3**
        ...
        for chunk in chunks:
            ...
            has_header = self._has_header_in_content(content)

            # If chunk has headers, header_path must be a string
            # NEW SEMANTICS: header_path can be empty if chunk only
            # has H3+ headers without structural context (H1/H2
            # ancestors)
            if has_header:
                # Check if chunk has structural headers (H1 or H2)
                has_structural_header = self._has_structural_header(content)

                # Only require non-empty header_path if chunk has
                # structural headers
                if has_structural_header:
                    assert (
                        header_path
                        and isinstance(header_path, str)
                        and header_path != "[]"
                    ), (
                        f"Chunk with structural headers (H1/H2) must "
                        f"have non-empty header_path string, "
                        f"got '{header_path}'"
                    )
                else:
            ...
            for adj_idx in [i - 1, i + 1]:
                if 0 <= adj_idx < len(chunks):
                    ...
                    if combined_size <= config.max_chunk_size:
                        ...
                        _ = header_path.strip("/").split("/")[:2]
                        _ = adj_path.strip("/").split("/")[:2]
