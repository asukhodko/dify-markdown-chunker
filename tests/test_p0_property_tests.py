...
class TestChunkConfigProperties:
    ...
    def test_property_chunk_config_round_trip(
        self, max_chunk_size, min_chunk_size, overlap_size, 
...
def valid_chunk_strategy():
    ...
    def _valid_chunk(draw):
        ...
        return Chunk(content=content, start_line=start_line, end_line=end_line, metadata=metadata)
...
class TestChunkingResultProperties:
    ...
    @settings(max_examples=50, deadline=5000)
    def test_property_chunking_result_round_trip(self, chunks, strategy_used, processing_time):
...
class TestChunkProperties:
    ...
    @given(
        ...
        st.dictionaries(
            ...
            values=st.one_of(st.text(max_size=50, alphabet='abcdefghijklmnopqrstuvwxyz'), st.integers(), st.booleans()),
            max_size=3
        )
    )
    @settings(max_examples=50, deadline=5000)
    def test_property_chunk_to_dict_includes_line_count(
        ...
        **Validates: Requirements 3.5**
        ...
        assert data['line_count'] == expected_line_count, \
