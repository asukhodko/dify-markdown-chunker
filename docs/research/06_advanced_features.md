...
**Description:** Использование sentence для определения оптимальных границ чанков на основе семантических переходов.
**How It Works:**
```python
...
"""
Find semantic boundaries using similarity.
...
if len(paragraphs) < 2:
return
= model.encode(paragraphs)
boundaries =
for i in range(len - 1):
similarity = cosine_similarity([i], [i+1])
