...
### Фаза 0: Подготовка тестового корпуса (1-2 дня)
- Создать корпус из 15+ реальных markdown документов
- Сохранить baseline результаты текущей реализации
- Создать скрипт сравнения результатов
- Определить количественные критерии rollback

### Фаза 1: Подготовка (2-3 дня)
- Создать property-based тесты для 10 доменных свойств
- Создать property-based тесты для 6 design fixes
...
### Фаза 5: Очистка и документация (3-4 дня)
- Удалить deprecated код
- Удалить backward compatibility
- Удалить старые тесты
- Создать MIGRATION.md с руководством по миграции
- Обновить документацию

## Общая оценка: 18-24 дня (включая Фазу 0)
